...
static bool checkContextFeatures(const char* label, bool bExtension, uint32_t availableCount, void* available,
  uint32_t requestedCount, const std::vector<const char*>& requestFeatures,
  ...
  *outEnableCount = 0;
  ...
  for (uint32_t i = 0; i < requestedCount; i++) {
    bool bFound = false;
    const char* result = "(required, not found)";
    ...
    for (uint32_t j = 0; j < availableCount; j++) {
      ...
      if (strcmp(availableName, requestFeatures[i]) == 0) {
    ...
    if (bFound) {
      result = "(required, enable)";
    }
    bFoundAllFeatures &= bFound;

    LOG_DEBUG("{0} {1}", requestFeatures[i], result);
  }
  LOG_DEBUG("-----------------------------");
